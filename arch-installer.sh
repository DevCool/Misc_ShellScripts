...
install_blackarch() {
    if [ "$INSTALLED" == "true" ]; then
        x=false
        if [ "$x" != "true" ]; then
            if (whiptail --title "Arch Linux Installer - Bl4cK" --defaultno --yesno "Would you like to install everything from BlackArch?" 10 60) then
		...
		arch-chroot "$ARCH" pacman -S --force --noconfirm blackarch &> /dev/null &
						...
						do
							...
							case "$GPU" in
								...
								"AMD")
									...
									"ati"	"Open source AMD driver" \
									"amdgpu"	"Open source AMD driver (Newer Cards)" 3>&1 1>&2 2>&3)
									if [ "$?" -eq "0" ]; then
										if [ "$multilib" == "true" ]; then
											query="xf86-video-$DRIV lib32-mesa"
										else
											query="xf86-video-$DRIV"
										fi
								...
								"Intel")
									...
									"intel"     "Open source Intel driver" 3>&1 1>&2 2>&3)
									if [ "$?" -eq "0" ]; then
										if [ "$multilib" == "true" ]; then
											query="xf86-video-$DRIV lib32-mesa"
										else
								...
								"VBOX")
									...
									"virtualbox" "Virtualbox Video Drivers" 3>&1 1>&2 2>&3)
									if [ "$?" -eq "0" ]; then
										query="$DRIV-guest-modules-arch"
				...
				case "$DE" in
					...
					"cinnamon") start_term="exec cinnamon-session"
						...
						if (whiptail --title "Arch Linux Installer" --yesno "Install SLiM display manager?" 10 60) then
							...
							DM="slim.service"
							if [ "$user_added" == "true" ]; then
								echo "exec cinnamon-session" > "$ARCH"/home/"$user"/.xinitrc
							else
								echo "exec cinnamon-session" > "$ARCH"/root/.xinitrc
							fi
						fi
					;;
					"gnome")
						if (whiptail --title "Arch Linux Installer" --yesno "Install gnome extras?" 10 60) then
							...
							extra_down=true post_down="$down" down=$(awk "BEGIN {print $down+4}")
						else
							envr="gnome"
						fi
						DM="gdm.service"
	...
	if [[ "$INSTALLED" == "true" && "$loader_installed" == "true" ]]; then
		if (whiptail --title "Arch Linux Installer" --yesno "Would you like to install some common software?" 10 60) then
			software=$(whiptail --title "Arch Linux Installer" --checklist "Choose your desired software \nUse spacebar to check/uncheck \npress enter when finished" 20 60 10 \
						...
						"xorg-xcalc"		"X Server Calculator" OFF \
						...
						"ufw"           	"Uncomplicated Firewall" OFF \
                                                "w3m"           	"Advanced Terminal Web Browser" OFF \
                                                "linux-headers" 	"Current Kernel Headers" ON \
